...
function TestPagination({examData,activeQuestion,changeActiveQuestion}) {
  ...
  return (
    <div className={styles["test__pagination"]}>
        ...
        <button type='button' className={`${styles["test__pagintaion-control-button"]} ${styles["test__next-button"]}`}
            onClick={() => changeActiveQuestion(activeQuestion >= examData.length ?activeQuestion:activeQuestion+1)}>
            <RightAnglePagIcon/>
        </button>
    </div>
  )
