const { ResponseSchema, MergeImageLink, SplitImageLink } = require('../../helper/HelperFunctions');
const { ErrorHandler } = require('../../helper/ErrorHandler');
const { GeneralSettings } = require('./GeneralSettingsModel');
const { GetGeneralSettings, AddGeneralSettings, UpdateGeneralSettings } = require('./GeneralSettingsService');

exports.updateSettings = async (req, res) => {
  const {is_project_In_factory_mode ,project_main_background_color ,project_main_text_color 
    ,project_whats_app_number,project_phone_number,project_email_address ,project_facebook_link
    ,project_twitter_link,project_instagram_link,shipping_chargers,vat_value ,project_logo:project_logo_body
  } = req.body;
  ...
  let addedData = {
    id: 1,
    project_logo: project_logo?.[0] ? project_logo?.[0]?.filename : project_logo_body ? SplitImageLink(req, project_logo_body) : '',
  ...
  try {
    ...
    return res.status(201).json(ResponseSchema(req.t('Settings Updated Successfully'), true));
...
exports.getSettings = async(req, res) => {
  try {
    const generalSetting = await GetGeneralSettings();
    const sendedObject = {
      project_logo: generalSetting?.project_logo ? MergeImageLink(req, generalSetting?.project_logo) : '',
      is_project_In_factory_mode: generalSetting?.is_project_In_factory_mode,
      project_main_background_color: generalSetting?.project_main_background_color,
    ...
    return res.status(200).json(ResponseSchema(req.t('Settings'), true, sendedObject));
  } catch (error) {
